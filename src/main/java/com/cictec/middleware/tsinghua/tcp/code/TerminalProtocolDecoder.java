...
public class TerminalProtocolDecoder extends CumulativeProtocolDecoder {
    ...
    protected TerminalMessage.Header decodeHeader(ByteBuffer in) throws EscapeException, IllegalMessageException {
        ...
        //消息ID
        header.setMessageId(BinaryUtils.wordToInt(in));
        //消息体长度
        header.setBodyLength(BinaryUtils.wordToInt(in));
        header.setTerminalId(BinaryUtils.byte2HexStr(in,8,false));
        header.setMessageSequence(BinaryUtils.wordToInt(in));
    ...
    private MessageDTO getMessageDTO(ByteBuffer in)  {
        ...
        try {
            ...
            //解析消息头
            ByteBuffer msgHeadBuffer = msgEscape(in, 14);
            TerminalMessage.Header header = decodeHeader(msgHeadBuffer);

            //解析消息体
            ByteBuffer msgBodyBuffer = msgEscape(in, header.getBodyLength());

            //校验码
            ByteBuffer checkDigitBuffer = msgEscape(in, 1);
            //校验码
