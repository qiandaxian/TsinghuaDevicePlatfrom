...
public class TerminalProtocolEncoder extends ProtocolEncoderAdapter {
	...
	public void encode(IoSession session, Object message,ProtocolEncoderOutput out) throws Exception {
		...
		logger.debug("发送消息 session {} 序号{} 消息ID {} ： {} ",session.getId(),header.getMessageSequence(), Integer.toHexString(header.getMessageId()) ,BinaryUtils.byte2HexStr(allBytes));
	...
	public byte[] encodeHeader(TerminalMessage.Header header){
		...
		if(sb.length()<14){
			for(int i=0;i<14-terminalId.length();i++){
				sb.insert(0, "0");
			}
		}
		if(sb.length()>14){
